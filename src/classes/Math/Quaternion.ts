...
export class Quaternion {
	...
	private static ANGLE_EPSILON: number = 1e-8;
	...
	static fromAxisAngles(axis: Vector3, angle: number): Quaternion {
		...
		let finalQuat: Quaternion = new Quaternion(
			cosAngle,
	...
	ToAxisAngles(this: Quaternion): AxisAngle {
		const angle: number = 2 * Math.acos(this.w);
		const devisor: number = Math.sqrt(1 - this.w * this.w);
		if (devisor < Quaternion.ANGLE_EPSILON) {
			return {
	...
	Inverse(this: Quaternion): Quaternion {
		const squaredNorm: number =
			this.x ** 2 + this.y ** 2 + this.z ** 2 + this.w ** 2;
		const inverted: Quaternion = new Quaternion(
			this.w / squaredNorm,
			-this.x / squaredNorm,
			-this.y / squaredNorm,
			-this.z / squaredNorm,
	...
	ConvertVectorToWorldSpace(this: Quaternion, vector: Vector3): Vector3 {
		const quatedVector: Quaternion = new Quaternion(
			0,
			vector.x,
			vector.y,
			vector.z,
		...
		const finalQuat: Quaternion = intermediate.MultiplyByQuaternion(
			this.Inverse(),
		);
	...
	ConvertVectorToLocalSpace(this: Quaternion, vector: Vector3): Vector3 {
		...
		const intermediate: Quaternion =
			this.Inverse().MultiplyByQuaternion(quatedVector);
