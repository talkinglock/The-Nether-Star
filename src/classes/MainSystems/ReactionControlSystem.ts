...
that is the responsibility of the Stability Augmentation And Redundent Collision Avoidance System (SARCAS)


*/
import { lstd } from "../../lua/bridge/lstd";
import { ShipBridge } from "../../lua/bridge/shipBridge";
import { SHIPBRIDGE_GETVELOCITY } from "../../lua/declaration/shipBridge";
...
export class ReactionControlSystem {
    ...
    private CanThrusterDecelerateSpin(this : ReactionControlSystem, thruster : Thruster, angularVelocity : Vector3): boolean
    {
        if (thruster.State === ThrusterState.Failed || thruster.State === ThrusterState.Emergency || thruster.State === ThrusterState.DisabledUnderEmergency) return false;
    ...
    // Activated by Failure Management System
    public EmergencySpinCondition(this : ReactionControlSystem): Recoverability
    {
        ...
        */
        ...
        if (PossibleDecelerationThrusters.length === 0 || NormalDecelerationThrusters.length === 0)
        {
            // it is literally impossible to decelerate
            return Recoverability.Unrecoverable;
        }   
        if ((PossibleDecelerationThrusters.length / NormalDecelerationThrusters.length) > this.fms.config.MinimumThrusterRotationAuthority)
        {
    ...
    private ThrusterLost(this : ReactionControlSystem, thruster : Thruster)
    {
        ...
        if (this.state == RCSState.Active)
        {
            lstd.Log("[RCS] Thruster Failed!")
            this.state = RCSState.Degraded;
        }
    }
    ...
    // THIS WILL ACTIVATE THRUSTERS FOR A FEW TICKS TO TEST THRUST!
    public PowerOnSelfTest(this : ReactionControlSystem): boolean {
        ...
        if (success === true)
        {
            this.state = RCSState.Active;
        }
        return success;
    }
    
    private CalculateThrusterTranslation(this : ReactionControlSystem, thruster : Thruster): number
    {
        const worldSpaceThrustVector : Vector3 = this.CurrentQuaternion.MultiplyByVector(thruster.ThrustVector);
        const dotToDesiredVector : number = worldSpaceThrustVector.Dot(this.DesiredVelocityNormalized);

        return (dotToDesiredVector * this.DesiredVelocityMagnitude);
    }

    private CalculateThrusterRotation(this : ReactionControlSystem, thruster : Thruster, globalRotationDotVector : Vector3): number
    {
        const alignment : number = thruster.Torque.Dot(globalRotationDotVector);
        return alignment;
    }
    
    private ThrusterUpdate(this : ReactionControlSystem, thruster : Thruster, globalRotationDotVector : Vector3)
    {
        /*
            3 steps per tick
        ...
        */
        ...
        const translationFactor : number = this.CalculateThrusterTranslation(thruster);
        const rotationFactor : number = this.CalculateThrusterRotation(thruster, globalRotationDotVector);
        let thrust : number = 0;
        ...
        {
            thrust = (translationFactor * this.fms.config.RCSFailMajorityMultiplier + rotationFactor * this.fms.config.RCSFailMinorityMultiplier) * 15;
        }
        else if (this.priority === RCSPriority.Rotation)
        {
            thrust = (translationFactor * this.fms.config.RCSFailMinorityMultiplier + rotationFactor * this.fms.config.RCSFailMajorityMultiplier) * 15;
        }
    ...
    // this function handles the calculations independent of the thrusters
    // for performance reasons
    UpdateRotation(this : ReactionControlSystem, currentQuat : Quaternion, errorQuat : Quaternion, SARCASRotationMultiplier : number): Vector3
    {
        const invertedQuat : Quaternion = currentQuat.Inverse();
        let worldError : Quaternion = invertedQuat.MultiplyByQuaternion(errorQuat);

        if (worldError.w < 0) 
        { 
            worldError = new Quaternion(-worldError.w, -worldError.x, -worldError.y, -worldError.z); 
        }

        const angle : number = 2 * Math.acos(worldError.w);
        const devisor : number = Math.sin(angle/2);
        const axis : Vector3 = new Vector3(worldError.x/devisor, worldError.y/devisor, worldError.z/devisor);
        ...
        const Adjusted : Vector3 = (axis.Multiply(angle).Subtract(localAngularVelocity));
        const finalVector : Vector3 = Adjusted.Multiply(SARCASRotationMultiplier);
        return finalVector;
    }


    Update(this : ReactionControlSystem, desiredVelocity: Vector3, desiredQuaternionAcceleration: Quaternion, SARCASRotationMultiplier : number) {
        if (this.state === RCSState.Offline) {
            throw new Failure("Attempt to use R.C.S pre-POST", FailureMode.RCS_POST_FAILURE);
        }
        if (desiredVelocity.Magnitude() === 0)
        {
            return;
        }
        if (this.state === RCSState.Failed)
        {
            return;
        }
        ...
        this.ErrorQuaternion = desiredQuaternionAcceleration;
        this.DesiredVelocity = desiredVelocity;
        this.DesiredVelocityMagnitude = desiredVelocity.Magnitude();
        this.DesiredVelocityNormalized = desiredVelocity.Normalized();

        const globalRotationDotVector : Vector3 = this.UpdateRotation(currentQuaternion, desiredQuaternionAcceleration, SARCASRotationMultiplier);

        this.Thrusters.forEach(thruster => {
            // thruster is type Thruster
            this.ThrusterUpdate(thruster, globalRotationDotVector);
        });
    }
